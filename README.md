...
- `didakta-{language}.xml` - TEI P5 edition, with a per-word vocabulary link on
  every example. Generated by `didakta_md2tei.py`; see
  ...
  Their vocabulary links have been repointed at Logeion by `didakta_relink.py`
...
- `didakta_md2tei.py` - builds the TEI editions from the markdown and the HTML export
- `didakta_relink.py` - repoints the vocabulary links of the published files at Logeion
...
by `didakta_md2tei.py`, which takes the text from the markdown and the link
structure from the HTML export:
...
`didakta_md2tei.py` emits Logeion links directly, so the TEI files need nothing
extra. The files downloaded from Zenodo are rewritten in place by:

```bash
python didakta_relink.py            # rewrite every file
python didakta_relink.py --check    # report what would change, touch nothing
```

It learns the betacode-to-Greek mapping from the HTML exports, where each link's
anchor text is the word as printed, so no link is rewritten on a guess; anything
unresolvable is left alone and reported. The script is idempotent — a second run
finds nothing to do. PDF pagination, EPUB and DOCX structure, and everything
other than the link targets are left exactly as published.
